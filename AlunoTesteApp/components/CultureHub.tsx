...
export const CultureHub: React.FC<CultureHubProps> = ({ language, apiKey }) => {
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setContent(response.text ?? "Sem conteúdo disponível.");
  ...
  const fetchTrends = async () => {
    ...
    try {
      ...
      setContent(response.text ?? "Sem tendências no momento.");
      extractLinks(response);
    } catch (e) {
