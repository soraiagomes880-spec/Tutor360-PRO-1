  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-700 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button 
          onClick={() => { setSearchQuery(''); fetchCultureData(); }}
          disabled={isLoading}
          className="flex items-center justify-center gap-2 px-5 py-2.5 bg-[#1e293b]/40 border border-white/10 rounded-xl hover:bg-white/10 transition-all text-slate-300 text-sm font-medium disabled:opacity-50"
        >
      ...
      <div className="glass-panel p-4 md:p-6 rounded-[2rem] md:rounded-[2.5rem] border-white/5 bg-[#0f172a]/50">
        <form onSubmit={handleSearch} className="flex flex-col sm:flex-row items-stretch gap-4">
          <div className="flex-1 flex items-center bg-black/40 rounded-2xl md:rounded-3xl border border-white/5 group focus-within:border-indigo-500/50 transition-all px-4 md:px-6 py-1">
            <div className="text-indigo-400 mr-3 shrink-0">
            ...
            <input 
              ...
              placeholder={`Explorar em ${language}...`}
          ...
          <button 
            ...
            className="px-6 py-4 bg-indigo-600 border border-indigo-500 text-white font-bold rounded-2xl transition-all shadow-lg text-xs flex items-center justify-center gap-2 hover:bg-indigo-500 disabled:opacity-30"
          >
        ...
        {isLoading ? (
          ...
          <div className="p-6 md:p-10 space-y-10 md:space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="text-center">
              <h3 className="text-[9px] md:text-[10px] font-black text-indigo-400 uppercase tracking-[0.4em] md:tracking-[0.5em] mb-8 md:mb-12 italic">Deep Dive Cultural</h3>
            </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-center opacity-40">
            ...
            <p className="text-slate-600 text-xs md:text-sm italic">Digite algo na barra acima para começar sua exploração cultural.</p>
          </div>
        )}
      </div>
    </div>
