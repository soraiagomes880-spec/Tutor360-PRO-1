    ...
    try {
      ...
      const sessionPromise = ai.live.connect({
        ...
        config: {
          ...
          systemInstruction: `Você é um tutor nativo especialista em ${language}. 
          SUA MISSÃO PRINCIPAL: Manter o aluno falando e imerso no idioma através de perguntas.
    ...
    <div className="h-full flex flex-col animate-in fade-in duration-700">
      ...
      <div className="mb-4 flex justify-between items-center h-16">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-xl font-bold text-white tracking-tight">Tutor 360 <span className="text-indigo-400">IA</span></h2>
        </div>
            ...
            <div className="flex gap-1 items-center">
              {[...Array(6)].map((_, i) => (
                <div 
                  ...
                  style={{ height: isConnecting ? '30%' : `${Math.max(20, audioLevel * 100)}%`, transition: 'height 0.1s ease-out' }}
                ></div>
              ))}
            </div>
            <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest animate-pulse">
              {isConnecting ? 'Conectando...' : 'IA Ativa'}
            </span>
          </div>
        )}
      </div>
      ...
      <div className="flex-1 glass-panel rounded-[2.5rem] border-white/10 flex flex-col overflow-hidden relative shadow-2xl bg-[#0a0f1d]">
        <div className="flex-1 p-8 overflow-y-auto space-y-6 flex flex-col scroll-smooth custom-scrollbar">
          {currentDisplayMessages.length === 0 && !isActive && !isConnecting && viewingHistoryIdx === null && (
            <div className="h-full flex flex-col items-center justify-center text-center p-12">
              <h3 className="text-3xl font-bold text-white mb-4">Escolha como começar</h3>
              <p className="text-slate-400 max-w-sm mb-12 text-sm leading-relaxed opacity-80">
                Pratique sua fala em <span className="font-bold text-white">{language}</span> por voz ou tire dúvidas digitando abaixo.
              </p>
        ...
        {/* Barra Inferior com Tradução e Input */}
          ...
          <div className="max-w-5xl mx-auto flex flex-col gap-4">
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex flex-col">
                  ...
                  <div className="flex items-center gap-3">
                    <select 
                      ...
                      className="bg-transparent border-none text-xs text-slate-300 font-medium outline-none cursor-pointer hover:text-white transition-colors"
                    >
                      {LANGUAGES.map(lang => (
              ...
              {(isActive || isConnecting) && (
                <button
                  onClick={stopSession}
                  className="px-6 py-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 text-[10px] font-bold uppercase tracking-widest rounded-xl border border-red-500/20 transition-all"
                >
                  Sair da Aula
                </button>
              )}
            ...
            <form onSubmit={handleTextResponse} className="relative w-full">
              <input 
                ...
                className="w-full bg-[#1e293b]/40 border border-white/10 rounded-2xl py-4 px-6 text-white placeholder-slate-600 outline-none focus:border-indigo-500/50 transition-all text-sm pr-28"
              />
              <button 
                type="submit"
                disabled={(!isActive && !isConnecting) || !userTextResponse.trim()}
                className="absolute right-3 top-1/2 -translate-y-1/2 px-5 py-2 bg-indigo-600/20 border border-indigo-500/30 text-indigo-400 hover:bg-indigo-600/30 hover:text-white disabled:opacity-20 text-xs font-bold rounded-xl transition-all"
              >
                Enviar
              </button>
            </form>
