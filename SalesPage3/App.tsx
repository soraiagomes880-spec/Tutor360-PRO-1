      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-5xl">
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-10">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 min-w-[200px]">
               <div className="flex justify-between items-center mb-2">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Uso do Plano</span>
                  <span className={`text-xs font-black ${usageCount > (MAX_USAGE * 0.8) ? 'text-red-500' : 'text-indigo-600'}`}>
                    {usageCount}/{MAX_USAGE}
                  </span>
               </div>
               <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                  <div 
                    className={`h-full transition-all duration-500 ${usageCount > (MAX_USAGE * 0.9) ? 'bg-red-500' : usageCount > (MAX_USAGE * 0.6) ? 'bg-amber-500' : 'bg-indigo-500'}`}
        ...
        <div className="mt-10 transition-all duration-300 relative">
          {isLimitReached ? (
            <div className="bg-white rounded-3xl p-12 text-center border-2 border-dashed border-indigo-200 shadow-xl animate-in zoom-in-95 duration-500">
               ...
               <h2 className="text-3xl font-black text-slate-800 mb-4">Limite de {MAX_USAGE} Usos Atingido!</h2>
               <p className="text-slate-500 max-w-md mx-auto mb-8">
                 Você aproveitou ao máximo seu tutor hoje. Para continuar praticando e desbloquear mais {MAX_USAGE} usos, clique no botão abaixo para renovar sua assinatura.
               </p>
               <button 
                onClick={renewSubscription}
