    ...
    <aside className="w-72 glass-panel border-r border-white/10 flex flex-col h-full z-30">
          ...
          {menuItems.map((item) => (
            ...
            >
              <div className="w-8 flex justify-center shrink-0">
                <i className={`fas ${item.icon} text-lg ${activeTab === item.id ? 'scale-110' : 'group-hover:scale-110 transition-transform'}`}></i>
              </div>
              <span className={`font-medium text-sm transition-all ${item.id === 'writing' ? '-ml-1.5' : ''}`}>
                {item.label}
              </span>
            </button>
          ))}
      ...
      <div className="p-8 border-t border-white/5">
        <div className="bg-gradient-to-br from-indigo-950/50 to-slate-900/50 rounded-2xl p-5 border border-white/10 shadow-lg">
          ...
          <p className="text-[10px] text-slate-500 leading-relaxed text-center italic">TESTE</p>
        </div>
      </div>
    </aside>
  );
